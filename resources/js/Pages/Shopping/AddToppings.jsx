...
const AddToppings = (params) => {
  ...
  const [toppingsBycate, setToppingsByCate] = useState([])

  useEffect(() => {
    filtrarCategoria(params.categorias[0].id)
  }, [])
  ...
  function filtrarCategoria(idCate) {
    const array = []
    params.adiciones.forEach(element1 => {
      if (element1.categoria == idCate) {
        array.push(element1)
      }
    })
    setToppingsByCate(array)
  }
  ...
  return (
    <div>
      ...
      <div style={{ textAlign: 'center', margin: '0.5em' }} className="container">
          ...
          {params.categorias.map((item, index) => {
            return (
              <div key={index} className='col-3'>
                <button onClick={() => filtrarCategoria(item.id)} className='btn btn-outline-success'>{item.nombre}</button>
              </div>
            )
          })}
        ...
        <div className="row justify-content-center" >
          {toppingsBycate.map((item, index) => {
            let img = params.globalVars.myUrl + 'Images/Config/nopreviewvertical.png'
            if (item.imagen != '') {
            ...
            return (
              <div key={index} style={{ marginTop: '0.5em' }} className="card border card-flyer pointer col-lg-3 col-md-3 col-sm-6 col-6">
                <img style={{ width: '65%', height: 'auto', marginTop: '0.2em' }} src={img} className="card-img-top img-fluid centerImg" alt="" />
                <h2 style={{ fontWeight: 'bold' }} id={'tituloTopping' + index} className="card-title">{item.nombre}</h2>
                <div className='row'>
                  <div style={{ marginLeft: '-0.6em' }} className='col-4'>
                  ...
                  <div style={{ marginTop: '0.3em', marginLeft: '0.5em' }} className='col-4'>
                  ...
                  <div style={{ marginLeft: '-0.5em' }} className='col-4'>
